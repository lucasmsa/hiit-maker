...
export const getNextExercises = (state: States) => {
  const { status, currentSetIndex, currentSetLoopIndex, currentSetExerciseIndex } = state.workoutExecution;
  if (status === WORKOUT_EXECUTION_STATUS.NOT_STARTED) {
    return [];
  } else if (status === WORKOUT_EXECUTION_STATUS.FINISH) {
    return [];
  } 
  const training = getTrainSetLoops(state);
  const currentTrainSetExercises = getCurrentSetExercises(training, currentSetIndex);
  const nextExercisesWithinCurrentSetLoop = currentSetLoopIndex + MAX_NEXT_EXERCISES < currentTrainSetExercises.length
  if (nextExercisesWithinCurrentSetLoop) {
    fetchCurrentTrainSetLoopNextExercises(currentTrainSetExercises, currentSetLoopIndex);
  }
...
const fetchCurrentTrainSetLoopNextExercises = (currentTrainSetExercises: Exercise[], currentSetLoopIndex: number) => {
  return currentTrainSetExercises.slice(currentSetLoopIndex + 1, currentSetLoopIndex + 1 + MAX_NEXT_EXERCISES).map(({name, image}) => ({ 
    name,
    image
  }));
}
